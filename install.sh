...
if echo "$OSTYPE" | grep -qE '^linux-gnu.*' && [ -f '/etc/debian_version' ]; then
	...
	printf "\r\033[1;31mUnrecognised OS.\e[0m Please follow 'Manual installation' at \033[0;94mhttps://github.com/HookDev-arch/Hikko/#-installation\e[0m"
	exit 1
fi
...
${SUDO_CMD}rm -rf Hook
# shellcheck disable=SC2086
runout ${SUDO_CMD}git clone https://github.com/HookDev-arch/Hikko/ || {
